...
describe("TeamStep", () => {
  it("should show the three preset teams and the no-team block when on the grid", () => {
    renderStep();
    expect(screen.getByText("小红书内容创作")).toBeTruthy();
    expect(screen.getByText("行业分析")).toBeTruthy();
    expect(screen.getByText("软件开发")).toBeTruthy();
  ...
  it("should reveal a preset's role briefs and deploy button when picked", () => {
    renderStep();
    fireEvent.click(screen.getByText("行业分析"));
  ...
  it("should call onEnter with the picked team id when entering the example project", async () => {
    ...
    fireEvent.click(screen.getByText("软件开发"));
    fireEvent.click(screen.getByRole("button", { name: /进入示例项目/ }));
    await waitFor(() => expect(onEnter).toHaveBeenCalledWith("development-engineering"));
  });
