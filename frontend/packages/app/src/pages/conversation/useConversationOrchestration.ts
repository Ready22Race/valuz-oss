...
import { useSkillStaging } from "./useSkillStaging";
import { useArtifactPane } from "./useArtifactPane";
import { useComposerConfig } from "./useComposerConfig";
import {
...
export interface ConversationOrchestrationParams {
   ...
   *  host page's own right panel — all global-panel writes and
   *  navigation-state consumption are suppressed. */
  variant: "page" | "panel";
  /** Current session id (``NEW_SESSION_ID`` sentinel or a real one) — owned
   *  by ``useConversationRouting``, NOT read from ``useParams`` here so this
   *  hook works identically whether or not there is a route. */
  id: string;
  conversationInstanceKey: string;
  /** Shared with ``useConversationRouting`` — set by ``useConversationSend``
  ...
  /** Client-declared host location for this conversation panel — threaded to
   *  ``sessionsApi.sendMessage``'s ``host_ref`` (direct-send path only). */
  hostRef?: SessionMessageHostRef | null;
  /** Seeds for a session that does not exist yet (``panel`` variant only —
...
/**
 * ── Conversation orchestration (the non-chrome 90%) ──────────────────
 *
 * Everything ``ConversationPage`` used to own directly EXCEPT the page
 * "chrome": ``useProjectOutlet()`` itself, ``ConversationHeader``,
 * ``ArtifactSplitPane``, ``useProjectHandoff``, ``useTitleActions``, and
 * ``useContextPanel`` — those stay in ``ConversationView``'s ``page``-variant
 * renderer, fed by this hook's return value. Bodies, comments, and
 * dependency arrays are moved verbatim from ``ConversationPage`` unless a
 * comment below says otherwise. The only structural adaptations:
 *
 *  - ``id`` / ``conversationInstanceKey`` / ``promotingSessionIdRef`` come in
 *    as params (``useConversationRouting`` owns the id-tracking effect that
 *    used to live inline here, reading straight off ``useParams``).
 *  - ``directoryFieldMode`` comes in as a param instead of destructuring the
 *    full ``useProjectOutlet()`` context (the other three outlet setters are
 *    chrome-only — never read outside ``useContextPanel``/``ConversationHeader``).
 *  - ``projectSendHandoffRef`` / ``handoffSessionIdRef`` / ``draftBootstrapSettled``
 *    are created HERE (``useProjectHandoff`` used to own them) since
 *    ``useConversationSend`` / ``useConversationHistory`` / ``useSessionSubscription``
 *    need them regardless of whether the page's project-handoff feature runs.
 *  - ``handleSend`` here is the same routing body ``useProjectHandoff`` used
 *    to own (busy → enqueue; parsing → confirm; else → send) — the ``page``
 *    variant still calls the real ``useProjectHandoff`` afterward and uses
 *    ITS ``handleSend`` (identical logic, plus the handoff-consumption
 *    effects) for parity; ``panel`` variant uses this one directly, since it
 *    never has a project-detail handoff to consume.
 */
export function useConversationOrchestration({
  variant,
  id,
  ...
  const panelSetCollapsedStore = usePanelStore((s) => s.setCollapsed);
  // The global right-panel (context/artifact pane) is PAGE chrome. In the
  // ``panel`` variant the conversation is itself hosted INSIDE that right
  // panel, so driving its collapsed state from session data would collapse
  // the very container we render in — every downstream consumer
  // (``useConversationHistory`` / ``useConversationSend`` / the session-change
  // effects below) goes through this guard instead of the raw store setter.
  const panelSetCollapsed = useCallback<typeof panelSetCollapsedStore>(
    (v) => {
      if (variant === "panel") return;
      panelSetCollapsedStore(v);
    },
    [variant, panelSetCollapsedStore],
  );
  ...
  // Project-detail handoff refs + the bootstrap-settled gate. Owned HERE
  // (rather than by ``useProjectHandoff``, which now only runs in the
  // ``page`` variant) since ``useConversationSend`` / ``useConversationHistory`` /
  // ``useSessionSubscription`` need working refs regardless of variant.
  ...
  // Artifact split pane — extracted to useArtifactPane. The ``page`` variant
  // wraps ``ConversationBody`` in ``ArtifactSplitPane`` using these; the
  // ``panel`` variant has nowhere to render the split view (345px), so it
  // just wires ``openArtifactFile`` through for citation/tool-card links.
  const {
    artifactFile,
    closeArtifact,
    openArtifactFile,
  ...
  // they write this hook's selectedRuntimeId/ProviderId/ModelId/Effort state.
  const {
    sessionExecOrigin,
  ...
  const {
    ...
    setRetryCounts,
    setKbPickerOpen,
    // The composer's send entry point — busy → enqueue; parsing → confirm;
    // else → send. Declared just below (deferred-closure pattern: this
    // function body references ``performEnqueue``/``displayBusy`` which are
    // ``const``s declared LATER via ``useInputQueue`` — safe because it is
    // only ever INVOKED post-render, from ``handleRetry``'s ``setTimeout`` or
    // a Composer click, by which point every referenced const is bound. See
    // ``useConversationSend.ts``'s own note on ``performSend``/``performEnqueue``
  ...
  const { subscribeToSession } = useSessionSubscription({
    ...
    currentClarifyingPendingRef,
    ruleIdToPreviewRef,
    // ``refreshQueueRef`` is declared further down this hook (block-
  ...
  useEffect(() => {
    // ``revealSessionContext`` rides on the ROUTE's location.state — in the
    // ``panel`` variant that state belongs to the host page, so consuming
    // (and stripping) it here would clobber the host's own navigation.
    if (variant === "panel") return;
  ...
  // ``panel`` variant's "starter" affordance: an external caller (finance's
  // panel head) fills the composer draft with example/prompt text. Not part
  // of the original ``ConversationPage`` — new for the embeddable view.
  useEffect(() => {
    if (prefillDraft === undefined || prefillDraft === null) return;
    setDraft(prefillDraft);
    onPrefillConsumed?.();
    // TODO(panel-composer-focus): ComposerPane doesn't expose an imperative
    // focus/caret handle yet, so this only seeds the draft text — it does not
    // focus the textarea or place the caret at the end. Follow up if the
  ...
  return {
    ...
    isNewSession,
    conversationInstanceKey,
    // chrome inputs some callers (page) still need to re-derive their own
    // chrome-only hooks (useProjectHandoff / useContextPanel / ConversationHeader)
    location,
    searchParams,
    fromTaskId,
    isSkillCreatorMode,
    ...
    pickerExpandFolder,
    handleKbPickerConfirm,
    // project KB / file tree (context panel)
    projectKbTree,
    projectKbBindings,
    handleExpandProjectKbFolder,
    fileTree,
