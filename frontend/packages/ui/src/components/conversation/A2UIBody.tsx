/**
 * Lazy boundary for the heavy A2UI rendering stack.
 *
 * Everything in this file — A2UIRenderer, @valuz/genui-blocks, @a2ui/react,
 * @a2ui/web_core, and @openuidev/react-ui (ThemeProvider + all components) —
 * is loaded on demand the first time a generate_ui card mounts, rather than
 * being part of the initial bundle. GenerativeUIRenderer provides a skeleton
 * fallback during the dynamic import.
 *
 * Exported as default because React.lazy() requires a default export.
 */
...
type GenerativeUIStatus = "running" | "success" | "error";
...
export interface A2UIBodyProps {
  body: string;
  status?: GenerativeUIStatus;
  hostParams?: Record<string, string | number | boolean>;
}

export default function A2UIBody({ body, status, hostParams }: A2UIBodyProps) {
  return (
    <OpenUITheme>
