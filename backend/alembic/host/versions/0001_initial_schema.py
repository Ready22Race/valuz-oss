...
The initial baseline of the host alembic chain — it creates the whole host
schema from empty. Released DBs are stamped at this revision (``0002``);
``run_host_migrations`` runs ``alembic upgrade head``, which is a no-op against
an already-stamped DB. Subsequent schema changes ship as NEW, reversible
revisions chained onto this one (``down_revision = "0002"``) so existing data
is migrated in place — never autogenerated back into this file and never
dropped. (The pre-launch "0-migration" baseline-regeneration + wholesale-reset
policy is retired; see ``boot.schema``.)
