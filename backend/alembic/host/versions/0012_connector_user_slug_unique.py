...
Downgrading restores the old global uniqueness invariant. If data already
relies on the new semantics (the same slug owned by multiple users), abort
with a clear error before rebuilding tables / creating constraints.
...
def _assert_no_cross_owner_duplicate_slugs() -> None:
    duplicates = (
        ...
        .execute(
            sa.text(
                "SELECT slug FROM valuz_connector GROUP BY slug HAVING COUNT(DISTINCT user_id) > 1"
            )
        )
        .fetchall()
    )
    ...
    raise RuntimeError(
        "Cannot downgrade connector slug uniqueness while slugs are shared "
