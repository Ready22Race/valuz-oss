...
The `node_modules/` tree is **not committed** (it's `.gitignore`d). Like the Node
binary, it's fetched at build time — committing ~350 third-party files bought
nothing once the Node binary moved to build-time download (no air-gapped build
either way). Integrity comes from the lockfile's per-dependency SHAs, verified by
`npm ci`.

## How it ships

`scripts/build-desktop.sh` (Phase A4) runs `npm ci --omit=dev` here, then stages
the resulting `node_modules/` into the desktop bundle at
`libexec/chrome-devtools-mcp/node_modules/`. `sidecar.ts` points the backend at
it via `VALUZ_CDT_ENTRY`
(`…/chrome-devtools-mcp/build/src/bin/chrome-devtools.js`), invoked with the
bundled `VALUZ_NODE_PATH`. With both env vars set the backend skips `npx`
entirely (see `modules/browser/service.py::_engine_argv`).
...
in sync with the same version, and re-run the browser smoke test.
