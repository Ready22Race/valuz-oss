#!/bin/sh
# supervisord program command for the Valuz kernel inside the Tencent AGS AIO
# sandbox. Self-migrates the kernel DB, then serves on loopback; the AIO nginx
# gateway (:8080) fronts it at /kernel/ and applies the sandbox's auth.
set -eu
...
# directly. That lets it run unauthenticated-on-loopback (the gateway's auth is
# the outer layer); set KERNEL_AUTH_TOKEN in the supervisord env instead if you
