...
One model for every history list: the project-home 全部/对话/任务/自动化 tabs
(``project_id`` set) and the global 动态 list (``project_id`` omitted). An item
is either a user chat session or a task entity — the entity view, so a task
keeps its own id/status/link and shows even before it spawns a lead session.
...
class ActivityItem(BaseModel):
    kind: str = Field(description='"chat" (a user session) or "task" (a task entity).')
    ...
    project_name: str | None = Field(
        default=None, description="null for non-project quick chats."
    )
    sort_at: int = Field(
        description="Unix epoch ms used for interleaving + the keyset cursor."
    )
