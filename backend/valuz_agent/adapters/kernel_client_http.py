...
class HttpKernelClient:
    ...
    async def set_mode(self, session_id: str, mode: str) -> SessionData:
        result = await self._request(
            "POST", f"/api/v1/sessions/{session_id}/mode", json_body={"mode": mode}
        )
        return SessionData(**result["data"])
    ...
    async def get_events_window(
        ...
        result = await self._request(
            "GET", f"/api/v1/sessions/{session_id}/events/window", params=params
        )
        return EventWindowData(**result["data"])

    async def subscribe_session_events(self, session_id: str) -> AsyncIterator[EventData]:
        async for item in self._stream_sse(f"/api/v1/sessions/{session_id}/events/stream"):
            yield item

    async def subscribe_all_events(self) -> AsyncIterator[EventData]:
        async for item in self._stream_sse("/api/v1/events/stream"):
            yield item

    async def _stream_sse(self, path: str) -> AsyncIterator[EventData]:
        try:
    ...
    async def usage_rollup(self, start_ms: int, end_ms: int) -> list[UsageRollupData]:
        result = await self._request(
            "GET", "/api/v1/usage", params={"start_ms": start_ms, "end_ms": end_ms}
        )
    ...
    async def interrupt(self, session_id: str) -> None:
        ...
        try:
            await self._request("POST", f"/api/v1/sessions/{session_id}/interrupt")
        except KernelSessionNotFoundError:
            return
    ...
    async def run_turn(
        ...
        messages = await self.list_messages(session_id, limit=1)
        if not messages:
