"""GenUI block registry — the backend half of dynamic block injection.

The frontend half (``@valuz/genui-blocks`` ``registerBlocks``) puts an
...
the other and the failure is silent in both directions — a described block
that renders as bare text, or a renderable block the model is never told
about — so editions register here from the same generated asset their
frontend install reads (see ``docs/design/genui-dynamic-blocks.md``).
...
* **Layer order mirrors the frontend.** Fixed ``commercial → distribution``;
  ...
  wholesale — but never another layer's blocks, and never the root — and only
  one layer may hold ``replace``, exactly as ``registerBlocks`` rules it.
"""
...
GenUIBlockLayer = Literal["commercial", "distribution"]
GenUIBlockMode = Literal["append", "replace"]

_LAYER_ORDER: tuple[GenUIBlockLayer, ...] = ("commercial", "distribution")
...
@dataclass
class GenUIBlockRegisterResult:
...
class _Registration:
    group: str
    #: ``(block name, pre-rendered catalog line)`` pairs. The line is authored
...
class GenUIBlockRegistry:
    ...
    The OSS baseline (the generated ``a2ui_block_catalog.txt`` plus the
    hand-listed OpenUI names) is bound lazily by ``modules/genui/protocol`` —
    ...
    def __init__(self) -> None:
        self._layers: dict[GenUIBlockLayer, _Registration] = {}
    ...
    def bind_baseline(self, *, names: Iterable[str], catalog_text: str) -> None:
        ...
        for layer, registration in list(self._layers.items()):
            ...
            if dropped:
                ...
                logger.error(
                    "genui blocks dropped at baseline bind (layer=%s): %s",
                    layer,
    ...
    def _split_baseline_collisions(
        ...
        for name, line in registration.entries:
            if name in self._baseline_names:
                dropped.append((name, "name is already taken by a built-in block"))
            else:
    ...
    def register(
        self,
        layer: GenUIBlockLayer,
        *,
        group: str,
        entries: Sequence[tuple[str, str]],
        notes: Sequence[str] = (),
        mode: GenUIBlockMode = "append",
    ) -> GenUIBlockRegisterResult:
        """Register a layer's blocks, replacing that layer's previous set.

        Taken names are refused rather than merged; the result's ``rejected``
        carries every refusal with its reason.
        """

        result = GenUIBlockRegisterResult()
        ...
        for name, line in entries:
            if not name:
                ...
                result.rejected.append(
                    (name, "name is already taken by a built-in, OpenUI, or another layer")
                )
            elif name in seen:
        ...
        if result.rejected:
            logger.error(
                "genui blocks rejected at registration (layer=%s): %s",
                layer,
    ...
    def unregister(self, layer: GenUIBlockLayer) -> None:
        self._layers.pop(layer, None)

    def _replacing_layer(self, except_layer: GenUIBlockLayer | None = None) -> str | None:
    ...
    def _taken_names(
        self, *, except_layer: GenUIBlockLayer, registering_mode: GenUIBlockMode
    ) -> frozenset[str]:
        names: set[str] = set()
    ...
    def baseline_suppressed(self) -> bool:
        """True when a layer holds ``replace`` — the built-in blocks (and the
        OpenUI vocabulary above them) must not be described to the model."""
    ...
    def catalog_text(self, *, baseline: bool = True) -> str:
        """The block half of the A2UI catalog: baseline ⧺ registered layers in
        ...
        for layer in _LAYER_ORDER:
            ...
            section = f"- {registration.group} blocks:\n{lines}"
