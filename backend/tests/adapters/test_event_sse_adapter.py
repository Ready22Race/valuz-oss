...
def test_should_translate_sandbox_status_when_host_reports_provision_start():
    """The allocator announces a cold boot before it blocks the turn — the one
    window where the session stream is otherwise silent."""
    result = _translate_kernel_event(
        "sandbox_status", {"phase": "starting", "scope": "session:sess-1"}
    )

    assert result is not None
    legacy_type, payload = result
    assert legacy_type == "session.sandbox_status"
    assert payload["phase"] == "starting"
    assert payload["scope"] == "session:sess-1"
    # No instance exists yet at ``starting`` — the keys are present but empty so
    # the wire shape is stable across both phases.
    assert payload["instance_id"] == ""
    assert payload["elapsed_ms"] == "0"


def test_should_translate_sandbox_status_when_host_reports_kernel_ready():
    result = _translate_kernel_event(
        "sandbox_status",
        {
            "phase": "ready",
    ...
    # Stringified per the legacy ``Record<string, string>`` SSE contract.
