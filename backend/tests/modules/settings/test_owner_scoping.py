...
The load-bearing property: the composite PK ``(key, user_id)`` means two users
can hold the *same* setting key without one's upsert clobbering the other's
(the single-column-PK bug this migration fixes).
"""
